...
/**************************************选台页面*************************************/
// 直播推屏
var pushScreenLive = function pushScreenLive(channelid) {
    console.log(channelid);
    pullAndPush.getDstUser(function () {
        var dstuserid = localStorage.getItem('dstuserid'),
            dstdeviceid = localStorage.getItem('dstdeviceid'),
            sessionid = new Date().getTime(),
            str = 'f02|{"actiontype":10102,"srcuserid":' + homed.getUserId() + ',"srcdeviceid":' + homed.getDeviceID() + ',"srchomeid":' + homed.getHomeId() + ',"dstuserid":' + dstuserid + ',"dstdeviceid":' + dstdeviceid + ',"dsthomeid":' + homed.getHomeId() + ',"sessionid":"' + sessionid + '","playmode":"http"' + ',"playstatus":1,"parameter":[{"channelid":' + channelid + '}] }';
...
var getVideoId = function getVideoId() {
    $.ajax({
        ...
        success: function success(res) {
            var channelHtml = '<li><span class="selected">全部</span></li>';
            ...
            try {
                for (var _iterator = tmp.data.liveCategoryList[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
//获取相应电视台列表
var getStationList = function getStationList(number) {
    $.ajax({
        ...
        success: function success(res) {
            console.log(JSON.parse(res));
            var data = JSON.parse(res).data.channelInfos;
            var listHtml = '';
            ...
            try {
                for (var _iterator2 = data[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                    ...
                    for (var ele in item.imageUrl[0]) {
...
function initPaint(vessel, canvas) {
    ...
    var array_paint = [],
        ...
        if (array_paint.length == 1) return;else {
            var i = 1;
